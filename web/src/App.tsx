...
export function App({
  ...
  const [onScreen, setOnScreen] = useState(() => {
    try {
      return localStorage.getItem('componium.roomPicture') === 'on';
    } catch {
      return false;
    }
  });
  ...
  const [projecting, setProjecting] = useState(() => {
    try {
      return localStorage.getItem('componium.roomProjection') === 'on';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    writeSetting('roomWash', wash);
  }, [wash]);
  useEffect(() => {
    writeSetting('roomLight', brightness);
  }, [brightness]);
    ...
    (async () => {
      try {
        const [s, r, m] = await Promise.all([
          fetch('/api/score').then((x) =>
            x.ok ? x.json() : Promise.reject(new Error('no score')),
          ),
          fetch('/api/rig')
            .then((x) => (x.ok ? x.json() : null))
            .catch(() => null),
  ...
  const loadLayout = useCallback(async () => {
    try {
      ...
      /* an arrangement is a convenience, never a blocker */
    }
  }, []);
  ...
  const preview = useCallback((instrument: string, level: number | null) => {
    setForced((was) => {
      ...
      if (level === null) next.delete(instrument);
      else next.set(instrument, level);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case '+':
        case '=':
          view.zoomAt(view.fractionOf(time), 0.6);
          onView();
          break;
        case '-':
        case '_':
          view.zoomAt(view.fractionOf(time), 1 / 0.6);
          onView();
          break;
        case 'f':
        case 'F':
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      void fetch('/api/layout', {
        ...
        /* losing an arrangement is not worth an error */
      });
    }, 600);
  ...
  const moveTo = useCallback(
    (instrument: string, before: string | null) => {
      setOrder((prev) => {
  ...
  const splitKeys = useCallback(
    (axis: 'x' | 'y', at: number, set: (v: number) => void, ends: [number, number], step: number) =>
